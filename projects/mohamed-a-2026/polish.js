...
if(document.body.hasAttribute('data-experience-booted'))loadGalleryStyles();else document.querySelector('[data-enter]')?.addEventListener('click',loadGalleryStyles,{once:true});
...
if(reduce||!('IntersectionObserver'in window)){zones.forEach(z=>{z.classList.add('is-live');z.dataset.choreographyFired='true'})}else{const fired=new WeakSet();const observer=new IntersectionObserver(entries=>{for(const entry of entries){if(entry.isIntersecting&&!fired.has(entry.target)){fired.add(entry.target);entry.target.dataset.choreographyFired='true';entry.target.classList.add('is-live');observer.unobserve(entry.target)}}},{threshold:.18,rootMargin:'-1% 0px -4%'});zones.forEach(z=>observer.observe(z))}
